...
// Education or Ministry of Justice result.
//
...
import { examBySlug } from "@/lib/is/registry";
import type { IcelandicSkill, IcelandicTaskType } from "@/lib/is/types";
...
export async function POST(req: Request): Promise<NextResponse> {
  ...
  const examName = exam?.name ?? "the Icelandic exam";
  ...
  const system = [
    `You are an experienced Icelandic-language examiner for ${examName} (CEFR ${cefr}).`,
    `You give an HONEST practice readiness estimate against the task's own criteria — this is a study aid, never an official Directorate of Education or Ministry of Justice result, and you never claim otherwise.`,
    isSpeaking
  ...
  if (body.itemId) {
    try {
      await prisma.icelandicAttempt.create({
        data: {
          userId: user.id,
          itemId: body.itemId,
          status: "EVALUATED",
