// Account page — plan + email status + target Icelandic exam.

import Link from "next/link";
import { redirect } from "next/navigation";
import type { IcelandicExam } from "@prisma/client";
...
async function setExam(formData: FormData) {
  ...
  await prisma.user.update({
    where: { id: user.id },
    data: { targetExam: valid ? (value as IcelandicExam) : null },
  });
    ...
    <div className="space-y-8">
      ...
      <section className="rounded-2xl border border-almi-bg-peach bg-almi-paper p-6 shadow-sm">
        ...
        {!proActive && (
          <p className="mt-3 text-sm text-almi-text-muted">
            Reading and Listening practice is free. Writing, Speaking and the full timed mock unlock
      ...
      <section className="rounded-2xl border border-almi-bg-peach bg-almi-paper p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-almi-text-muted">
          Pick the Ríkisborgarapróf (citizenship, A2), the University of Iceland entrance exam, or a
          general CEFR level (A1–C1). This sets the default for your practice and full mock. You can
          change it any time.
        </p>
        <form action={setExam} className="mt-4 flex flex-wrap items-center gap-3">
          ...
          >
            ...
            <optgroup label="Citizenship — Ríkisborgarapróf">
            ...
            <optgroup label="University of Iceland entrance">
            ...
            <optgroup label="General Icelandic (CEFR)">
              {ALL_EXAMS.filter((e) => e.track === "CEFR").map((e) => (
                <option key={e.exam} value={e.exam}>
                  {e.name} · {e.cefr}
                </option>
              ))}
        ...
        <p className="mt-2 text-xs text-almi-text-muted">
          Not sure which exam you need? Confirm citizenship requirements with Útlendingastofnun, and
          entrance requirements with the University of Iceland (Háskóli Íslands).
        </p>
      </section>
      ...
      <section className="rounded-2xl border border-almi-bg-peach bg-almi-bg p-6 text-center">
        <p className="text-sm text-almi-text">Ready to practise Icelandic?</p>
        <Link
          href="/practice"
