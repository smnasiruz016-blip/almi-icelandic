...
export default async function sitemap({ id }: { id: Promise<string> }): Promise<MetadataRoute.Sitemap> {
  ...
  // Shard 0 — core marketing + auth + Icelandic-exam hub/levels.
  if (shard === 0) {
    const core = ["", "/pricing", "/signup", "/login", "/exams", "/study-in-iceland", "/work-in-iceland", "/requirements/iceland/rikisborgaraprof-citizenship"]
