...
/** The BCP-47 voice tag for listening audio — Icelandic (all tracks). */
...
export async function submitAttempt(body: unknown): Promise<SubmitResult | null> {
  try {
    const res = await fetch("/api/is/submit", {
      method: "POST",
...
export async function gradeProductive(body: {
   ...
   *  level (via almi-data's levelInstruction), never at the exam's display label. */
  cefr?: CefrLevel;
  title: string;
  prompt: string;
  criteria: string[];
  ...
  try {
    const res = await fetch("/api/is/grade", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    });
