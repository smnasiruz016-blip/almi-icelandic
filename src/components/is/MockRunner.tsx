...
export function MockRunner({
  ...
  async function finish() {
    setGrading(true);
    const perSkill: Partial<Record<IcelandicSkill, { points: number; max: number }>> = {};
    const prodPct: Partial<Record<IcelandicSkill, number[]>> = {};
...
function MockResult({
  ...
  agg: { meanPct: number; overall: string; label: string; weakest: IcelandicSkill | null; allClear: boolean };
}) {
  return (
    <div className="space-y-5 rounded-2xl border border-almi-bg-peach bg-almi-paper p-6">
      ...
      <p className="text-xs text-almi-text-muted">
        This is a practice estimate, not an official Directorate of Education or Ministry of Justice
