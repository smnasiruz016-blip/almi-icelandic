...
//
//  1. SELF-ORIGIN. src/data/seo/countries.json listed Iceland itself, so the
//     matrix advertised "Study in Iceland from Iceland". almi-dutch and
//     almi-icelandic had the identical bug. axes.ts even carried the comment
//     "origins only (Iceland excluded)" — the label was localized, the fact was
...
const SELF_ISO = "IS"; // this product's own country — never a valid origin of itself
