...
import { getItems } from "../../src/lib/is/items";
import { ALL_EXAMS } from "../../src/lib/is/registry";
import type { IcelandicSkill } from "../../src/lib/is/types";
...
for (const exam of ALL_EXAMS) {
  const counts = exam.skills.map((skill) => {
    const n = getItems({ exam: exam.exam, skill: skill as IcelandicSkill }).length;
    modules++;
    total += n;
